...
class TestOpenwakewordImportGuard(unittest.TestCase):
    """A missing openwakeword must raise ImportError with the two-command
    workaround, not an opaque ModuleNotFoundError from the bare import."""

    def _import_without_openwakeword(self):
        mod = importlib.import_module("ovos_ww_plugin_openwakeword")
        # a failed reload leaves the module half-initialized in sys.modules;
        # restore the working module afterwards so later tests are unaffected
        self.addCleanup(importlib.reload, mod)
        with patch.dict(sys.modules, {"openwakeword": None, "openwakeword.utils": None}):
            # patch.dict with None makes `import openwakeword` raise
            # ImportError even if it is installed in this interpreter.
            importlib.reload(mod)

    def test_missing_openwakeword_raises_importerror_with_workaround(self):
        with self.assertRaises(ImportError) as ctx:
            self._import_without_openwakeword()
        msg = str(ctx.exception)
        self.assertIn('pip install --no-deps "openwakeword>=0.5.0,<1"', msg)
        self.assertIn('pip install "ovos-ww-plugin-openwakeword[onnx]"', msg)

    def test_missing_openwakeword_names_py312_cause(self):
        with self.assertRaises(ImportError) as ctx:
            self._import_without_openwakeword()
