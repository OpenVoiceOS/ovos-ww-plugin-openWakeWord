...
# Linux, and tflite-runtime has no wheels for Python >= 3.12, so a plain
# `pip install openwakeword` fails to resolve there. Distinguish "package
# absent" from "package present but its own dependencies absent" (the state
# between the two workaround commands below), and raise with the matching
# workaround instead of an opaque ModuleNotFoundError.
try:
    import openwakeword
    ...
    if importlib.util.find_spec("openwakeword") is None:
        raise ImportError(
            "The 'openwakeword' package is not installed. On Python >= 3.12 a "
            "plain install fails because openwakeword requires tflite-runtime, "
            "which has no wheels for this interpreter. Install it with:\n"
            '  pip install --no-deps "openwakeword>=0.5.0,<1"\n'
            '  pip install "ovos-ww-plugin-openwakeword[onnx]"'
        ) from None
    raise ImportError(
        "The 'openwakeword' package is installed but its runtime dependencies "
        "are missing (importing it raised an ImportError). Install them with:\n"
        '  pip install "ovos-ww-plugin-openwakeword[onnx]"'
    ) from None
...
def _tflite_available() -> bool:
    ...
    frequently installed without it. openwakeword (>=0.6.0) tolerates this at
    runtime: it falls back to onnx inference on its own, whatever
    `inference_framework` is requested, so this plugin only uses the check to
    pick a sensible default framework.
    """
    try:
